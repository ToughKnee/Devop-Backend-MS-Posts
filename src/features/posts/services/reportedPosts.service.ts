import { getReportedPostsPaginated, getTotalReportedPosts } from '../repositories/reported.posts.repository';
 ...
 */
export const getReportedPosts = async (page: number, limit: number) => {
  try {
    const offset = (page - 1) * limit;
    
    const totalPosts = await getTotalReportedPosts();
    const totalPages = Math.ceil(totalPosts / limit);

    const posts = await getReportedPostsPaginated(offset, limit);
